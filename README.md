...
- תצוגת תגובות בסגנון צ'אט: כל תגובה מוצגת במסגרת נפרדת עם שם המגיב, תאריך ושעת כתיבה.
...
- React 17
- Vite
- React Router עם HashRouter לשמירת נתיבים בזמן רענון
- Axios
- React Icons
- React Spinners
...
תגובות חדשות נשמרות במתכון כמערך של אובייקטים. כך אפשר להציג לכל תגובה גם את התוכן, גם את שם המגיב וגם את זמן הכתיבה:
...
השרת יוצר את השדות האלה בזמן הוספת תגובה:

- `text` הוא תוכן התגובה שהמשתמש כתב.
- `authorName` נוצר מהחלק הראשון של כתובת המייל לפני הסימן `@`.
- `authorId` נשמר לפי מזהה המשתמש המאומת.
- `createdAt` נוצר בצד השרת כדי שהזמן לא ייקבע על ידי הדפדפן.

עמוד פרטי המתכון תומך גם בתגובות ישנות שנשמרו בעבר כטקסט פשוט, כדי למנוע שבירה אם נשאר מידע ישן במסד הנתונים.
...
- בדיקה שעמוד פרטי מתכון מציג תגובה עם שם מגיב, תאריך ושעת כתיבה.
...
הלקוח משתמש ב-`HashRouter`, ולכן כתובות פנימיות נשמרות אחרי הסימן `#`. לדוגמה:
...
בצורה הזאת רענון בדפדפן מבקש מהשרת רק את `/`, ו-React Router ממשיך לטעון את העמוד הנכון בצד הלקוח.

אם עוברים בעתיד בחזרה ל-`BrowserRouter`, צריך שכתובות פנימיות כמו `/Details/:id`, `/Categories/:category` ונתיבים לא קיימים ייטענו דרך React Router. במקרה כזה יש להגדיר Rewrite ב-Static Site:
...
אם השירות ב-Render לא נוצר מתוך `render.yaml`, צריך להוסיף את אותו כלל ידנית בדשבורד של Render תחת הגדרות ה-Static Site. בלי הכלל הזה, `BrowserRouter` עלול להחזיר `Not Found` בכניסה ישירה לנתיב כמו `/fssdasd`. השימוש הנוכחי ב-`HashRouter` מונע את הבעיה הזאת ברענון, כי הנתיב נשאר בצד הדפדפן אחרי `#`.
