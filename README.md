...
אם השירות ב-Render לא נוצר מתוך `render.yaml`, צריך להוסיף את אותו כלל ידנית בדשבורד של Render תחת הגדרות ה-Static Site. בלי הכלל הזה, כניסה ישירה לנתיב כמו `/fssdasd` עלולה להחזיר `Not Found` במקום להציג את עמוד `PageNotFound` של React.
